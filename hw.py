...
scenarios0 = [("1 1 4 1 6 5 -2 -2 3 -1 1 3", "false"),
              ("-2.14 3.2 -0.18 -2.4 3.48 2.84 -0.98 0.6 0.98 -5 4.64 0.24",
               "true"),
              ("0.1 0.3 6.1 -2.88 7.3 -5.74 -0.5 -0.03 7.3 -4.16 8.86 -7.88",
               "true")]
...
scenarios3 = [("", "2 3 5 7 11 13 17 31 37 71 73 79 97 113 131 197 199 311 \
              337 373 719 733 919 971 991")]
...
for root, dirs, files in os.walk(directory, topdown=False):
    if os.path.basename(root).lower() == homework_name.lower():
        ...
        files = [f for f in listdir(root) if (path.isfile(path.join(root, f))
                 and f.endswith(".cpp")) and f == program_name]
            ...
            for scenario in scenarios3:
                ...
                if output == scenario[1]:
                    ...
                    sys.stdout.write(" ".join(" {0} error ({1} != {2})"
                       .format(scenario, scenario[1], output).split()))
                    successful = False
